...
Properties {
    ...
    $buildconfig = 'Debug'
    # deployconfig: the configuration used for generated config files
    $deployconfig = 'user'
    # repos: ordered array of nuget repositories to use for fetching packages
    $repos = @('local', 'externals', 'ppwcode-debug', 'pensiob-debug', 'nuget')
...
function PropertyDocumentation() {
    ...
    Write-Host "`$deployconfig    Configuration for generated configuration files."
...
Task Build -description 'Build the solution.' -depends Clean {
    ...
    try
    {
        Set-Location 'src'
        $solution = Get-Item '*.sln' | Select-Object -First 1
        Chatter "MSBuild: $($solution.Name) /t:Build /m /nr:false /p:Configuration=$buildconfig /p:PPWTEMPLATECONFIG=$deployconfig /v:quiet /nologo." 3
        Exec { msbuild "$($solution.Name)" /t:Build /m /nr:false /p:Configuration=$buildconfig /p:PPWTEMPLATECONFIG=$deployconfig /v:quiet /nologo }
    }
    finally
    {
        Pop-Location
...
Task Clean -description 'Clean build output and generated packages.' {
    ...
    try
    {
        ...
        Set-Location 'src'
        $solution = Get-Item '*.sln' | Select-Object -First 1
        Chatter "MSBuild: $($solution.Name) /t:Clean /m /nr:false /p:Configuration=$buildconfig /p:PPWTEMPLATECONFIG=$deployconfig /v:quiet /nologo." 3
        Exec { msbuild "$($solution.Name)" /t:Clean /m /nr:false /p:Configuration=$buildconfig /p:PPWTEMPLATECONFIG=$deployconfig /v:quiet /nologo }
        Set-Location '..'

        # clean up scratch folder
        Chatter 'Cleaning scratch' 2
        Remove-Item -Path 'scratc[h]' -Recurse -Force
...
Task Documentation -description 'Generate documentation using Sandcastle Help File Builder.' -depends FullBuild {
    ...
    try
    {
        ...
        $shfbfiles | ForEach-Object {
            ...
            Set-Location $_.DirectoryName
            Exec { msbuild "$($_.Name)" /t:Build /nr:false /p:Configuration=$buildconfig /p:PPWTEMPLATECONFIG=$deployconfig /v:quiet /nologo }
        }
    }
    finally
    {
